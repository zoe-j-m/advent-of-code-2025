...
import { DimensionsError, matrix } from '../utils/matrices.ts'
...
describe('matrix', () => {
  ...
  describe('forEach (overloads)', () => {
    ...
    it('should return a new matrix of a different type', () => {
      const newMatrix = m2d.forEach(
        (value, coords) => `${value}@${coords.join(',')}`
      )!
  ...
  //         [3, 4],
  //     ]);
  //     const result = m2d.rotate('Left')
  //     expect(result.get([])).toEqual([[2,4],[1,3]])
  // })})
})
